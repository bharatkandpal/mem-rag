...
- [ ] RAG-66 — **`rag init` scaffolding generator — design + build**: target detection, the file set written, `RagModule.forRoot()` config surface, idempotent re-run, and the importable-package surface. **Slice before building** (`task-slice`).  · GO-21j, `cli` + `nest-module` skills → sliced (see subtasks/GO-21j.md) — **RAG-66b + RAG-66c done 2026-07-29** (importable package surface + typed `forRoot(options)` override surface: `embeddingProvider`/`generationProvider`/`k`/`minScore`/`http`, all verified end-to-end; along the way, found + fixed a real Nest module-metadata-concatenation gotcha — see `doc/LEARNINGS.md`). Next: RAG-66d (`rag init` scaffold writer).
