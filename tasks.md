...
- [ ] RAG-66 — **`rag init` scaffolding generator — design + build**: target detection, the file set written, `RagModule.forRoot()` config surface, idempotent re-run, and the importable-package surface. **Slice before building** (`task-slice`).  · GO-21j, `cli` + `nest-module` skills → sliced (see subtasks/GO-21j.md) — **RAG-66a–d done 2026-07-29** (approach guide, importable package surface, typed `forRoot(options)` override surface, and the `rag init` scaffold writer — `src/cli/init.ts`, writes `src/rag/rag.module.ts`/`.env.rag.example`/`docker-compose.rag.yml`/`db/rag/001_init.sql`, idempotent + `--force`/`--dry-run`, live-smoke-verified via `node dist/cli/main.js init` and `npx rag init`). Bonus find: RAG-46's migration runner already supports a `MIGRATIONS_DIR` override, so a scaffolded host can apply the generated schema with zero new runner code — documented in the generated SQL. Next: RAG-66e (verify the DB path end-to-end) + RAG-66f (`npm run rag` script + README).
