...
>
> 🧭 **Design doc drafted 2026-07-29** → accepted design guide [`docs/mcp-layer-guide.md`](docs/mcp-layer-guide.md): D1–D5 resolved (both structured+text results · `rag_ingest` behind `MCP_ENABLE_INGEST` · stderr logging via the RAG-63 `CorrelatedLogger` · stdio no-auth / HTTP bearer · no eval, `answer-eval` after wiring), pinned against the real `QueryResult` (`abstained`+`grounded`) and the current Anthropic MCP connector shapes. Ready to slice into RAG-65a…f (build slices listed in §8). **RAG-65a done 2026-08-03** — `src/mcp/main.ts` app-context entrypoint (mirrors the CLI) serving an empty `McpServer` (`@modelcontextprotocol/sdk@1.30.0`, exact-pinned) over stdio; logs forced to stderr (D3) so stdout is protocol-only; `npm run mcp`; live-verified via an `initialize` handshake (`serverInfo {name:"rag"}` on stdout, readiness on stderr). **RAG-65b done 2026-08-03** — `rag_query` tool (`src/mcp/rag-query.tool.ts` + `render.ts` + 4 unit tests): calls `GenerationService.generate`, returns structured `QueryResult` (Zod `outputSchema`, D1) + a text rendering (answer + trailing numbered `Sources:` list; abstain verbatim, non-citation provider → capability note). Deviation: trailing source list, not inline `[n]` (Citation carries the source span, not an answer offset). Live-verified via a real stdio MCP `Client` over the seeded corpus + qwen2.5:3b — grounded (`grounded:true`, chunk + answer) and abstain (`abstained:true`/`grounded:false`, verbatim). **RAG-65c done 2026-08-04** — citation-serialization unit tests (`src/mcp/rag-query.tool.spec.ts`, 7 tests): captures the tool handler via stubs and asserts both provider types serialize faithfully through MCP (Anthropic → mapped `citations[]` + `Sources:` text; openai-compatible → `citationsSupported:false`+`[]`, no `[n]` markers, capability note — never fabricated; abstain pass-through). 149 tests green. **RAG-65d done 2026-08-04** — gated `rag_ingest` (`src/mcp/rag-ingest.tool.ts` + `register-tools.ts` aggregator holding the `MCP_ENABLE_INGEST` gate, default off; new env var in `.env.example`; +4 unit tests). Live-verified via a real stdio MCP `Client`: flag off → `tools/list` = `[rag_query]` (ingest absent); flag on → `rag_ingest` present + real ingest `{docs:4,chunks:9}`, idempotent re-run stable at 9. 153 tests green. **RAG-65e done 2026-08-04** — Streamable HTTP transport + bearer auth (`src/mcp/http-transport.ts` self-contained `/mcp` listener, stateless per-request; `src/mcp/config.ts` `resolveMcpServerConfig`, fail-closed — http without a token throws; `main.ts` branches on `MCP_TRANSPORT`; new env `MCP_TRANSPORT`/`MCP_HTTP_PORT`/`MCP_AUTH_TOKEN`; +13 unit tests). Live-verified: curl → 401 (no/wrong token) / 404 (off-path); SDK `StreamableHTTPClientTransport` + bearer → `tools/list` + grounded/abstain `rag_query` over HTTP; wrong bearer rejected. The hosted Anthropic-connector hop needs a public URL + keys (GO-21f). **RAG-65f done 2026-08-04** — answer-eval through the `rag_query` tool: 10/10 answerable grounded, 4/6 abstain (reproduces the documented direct-`/query` baseline incl. the same RAG-57 leaks — the proof serialization preserves outcomes); README §8 "Expose the corpus to AI agents (MCP)" + three-entrypoints architecture; LEARNINGS wrap-up. **✅ GO-21i / RAG-65 COMPLETE (RAG-65a…f).** 166 tests green.

- [x] RAG-65 — **MCP layer** (tool surface, service reuse, transport, auth, citation→MCP mapping, abstain semantics).  · new capability, `add-adapter`-style seam → **design doc delivered** ([`docs/mcp-layer-guide.md`](docs/mcp-layer-guide.md)) + **fully built RAG-65a…f (done 2026-08-04)**: `src/mcp/` third entrypoint (`npm run mcp`) over the same in-process services — stdio + Streamable-HTTP transports (bearer auth, fail-closed), `rag_query` (structured `QueryResult` + text) + gated `rag_ingest` (`MCP_ENABLE_INGEST`), citation-serialization tests for both provider types, and an answer-eval pass through the tool (10/10 answerable grounded, abstain matches the direct path). README §8 documents it. 166 tests green. **GO-21i complete.**
- [ ] RAG-69 — **Per-agent / OAuth auth for the MCP HTTP transport** — **NOT IN SCOPE (future release).** The shipped HTTP transport uses a single static shared bearer token (`MCP_AUTH_TOKEN`, RAG-65e) — authentication only, no per-agent identity, scopes, or token rotation. A future iteration could adopt the MCP SDK's OAuth `authProvider` flow for per-agent identity + scoped access (e.g. gating `rag_ingest` per caller rather than the global `MCP_ENABLE_INGEST` flag). Deferred deliberately: a shared bearer is the right weight for the current single-tenant use; revisit if multi-tenant / untrusted-agent hosting becomes a requirement.  · design guide D4, `ai-and-secrets.md`
