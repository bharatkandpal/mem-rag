...
## Validating a pass-through end-to-end: reproduce the baseline, don't chase a better number (RAG-65f)

### "Survives serialization" means the same outcome as the direct path — including the same known failures
**Concept:** The answer-eval for the MCP layer ran all 16 questions through the `rag_query` tool and compared the grounded/abstain classification to the documented direct-`/query` baseline. The pass condition was **reproduction, not improvement**: 10/10 answerable stay grounded, and abstain lands at 4/6 — with the *same two* tech-adjacent leaks (k8s ingress, React useEffect) that RAG-57 already documented as scoring above the floor. If the MCP path had shown 6/6 abstain, that would have been a red flag, not a win — it would mean the transport was somehow altering results the direct path doesn't. The right target for a pass-through layer is "identical to the layer beneath," bit-for-bit including its warts.
**Why it matters:** It's tempting to treat any eval as "higher is better," but for a transport the metric is *fidelity*, not quality. The known leaks are a feature of the test here: seeing them survive the MCP hop is positive evidence that nothing in serialization changed the classification. A layer that "improved" the number would actually be suspect.

### The keyless environment splits the eval cleanly along the axis that needs keys
**Concept:** The answer-eval skill has three axes — groundedness (LLM-judge), citation accuracy (LLM-judge, and only when `citationsSupported`), and abstention-correctness (structural). The first two need `claude-opus-4-8` as judge (Anthropic keys) and native citations (also keys); the third needs neither. RAG-65f's actual claim — grounding/abstain survive serialization — is exactly the abstention-correctness axis, so it runs fully keyless through the local stack, while the judge-scored axes are left to a keyed run + the structural serialization already proven by the RAG-65c unit tests. Rather than fake a judge or skip verification, the eval was scoped to the axis the environment can actually run honestly.
**Why it matters:** When you can't run the full battery, the move is to identify which part of it your claim actually depends on and run that part for real — not to run a degraded version of the whole thing and call it done. Here the milestone's guarantee maps precisely onto the one axis that doesn't need keys, so the keyless run is a complete proof of *that* claim, with the keyed axes honestly deferred.
