...
### `src/mcp/main.ts`
- **Purpose:** The MCP server entrypoint (GO-21i / RAG-65a) — a third entrypoint over the same in-process services as the HTTP API and CLI. Bootstraps `NestFactory.createApplicationContext(AppModule, { logger: false })` (same pattern as `src/cli/main.ts`), forces logs to **stderr** via `CorrelatedLogger('stderr')` so stdout stays a pure JSON-RPC channel (design guide D3), resolves `GenerationService` at boot (fail-fast on a broken DI graph), and serves an empty `McpServer` (`@modelcontextprotocol/sdk`, v1.30.0) over `StdioServerTransport`. SIGINT/SIGTERM → clean shutdown (`server.close()` + `app.close()`). Registered as `npm run mcp` (`node dist/mcp/main.js`). **Tools land later:** `rag_query` (RAG-65b), gated `rag_ingest` (RAG-65d), HTTP transport + bearer auth (RAG-65e).
- **Defines:** `bootstrap(): Promise<void>` (self-invoked) · `SERVER_INFO` (file-private const `{ name:'rag', version:'0.1.0' }`)
- **Depends on:** `AppModule`, `GenerationService`, `CorrelatedLogger` (`../observability/correlated-logger`), `Logger` (`@nestjs/common`), `NestFactory`, `McpServer` (`@modelcontextprotocol/sdk/server/mcp.js`), `StdioServerTransport` (`@modelcontextprotocol/sdk/server/stdio.js`), `registerQueryTool` (`./rag-query.tool`)
