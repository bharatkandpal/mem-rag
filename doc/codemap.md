...
>
> **Last updated:** RAG-65e Streamable HTTP transport + bearer auth — new `src/mcp/config.ts` (`resolveMcpServerConfig` — pure, reads `MCP_TRANSPORT`/`MCP_HTTP_PORT`/`MCP_AUTH_TOKEN`, **fail-closed**: `http` without a token throws) and `src/mcp/http-transport.ts` (`startHttpServer` — self-contained `http.createServer` on `/mcp`, `StreamableHTTPServerTransport` stateless per-request; `isAuthorized` constant-time bearer check, 401 on miss/mismatch, 404 off-path). `src/mcp/main.ts` now branches on transport (stdio vs http) and validates via `resolveMcpServerConfig`. New env `MCP_TRANSPORT`/`MCP_HTTP_PORT`/`MCP_AUTH_TOKEN` (in `.env.example`). 13 unit tests (config resolution + bearer). Live-verified: curl → 401 (no/wrong token), 404 (off-path); SDK `StreamableHTTPClientTransport` with correct bearer → `tools/list`, grounded `rag_query` (`grounded:true`+chunk) and abstain over HTTP; wrong bearer rejected at connect. Prior: RAG-65d gated `rag_ingest` MCP tool — new `src/mcp/rag-ingest.tool.ts` (`registerIngestTool`) exposes corpus ingest via `IngestionService.ingest` (input `{path}`, output `{docs,chunks,ms}`); new `src/mcp/register-tools.ts` (`registerTools`) is the tool aggregator holding the **`MCP_ENABLE_INGEST` gate** (D2) — `rag_query` always on, `rag_ingest` only when the flag is true (a pure fn, unit-testable without Nest). `src/mcp/main.ts` now resolves `IngestionService` + `ConfigService`, reads `MCP_ENABLE_INGEST` (default false), and calls `registerTools`. New env var `MCP_ENABLE_INGEST`; `.env.example` documents it. Live-verified via a real stdio MCP `Client`: flag off → `tools/list` = `[rag_query]` (rag_ingest absent); flag on → `rag_ingest` present + a real ingest returned `{docs:4,chunks:9}`, idempotent re-run stable at 9. Prior: RAG-65c MCP citation-serialization tests — new `src/mcp/rag-query.tool.spec.ts` (7 tests): captures the registered tool handler via stub `McpServer`+`GenerationService` and asserts both provider types serialize faithfully through MCP — Anthropic → `citations[]` mapped to sources in `structuredContent` + numbered `Sources:` text; openai-compatible → `citationsSupported:false`+`citations:[]`, no `[n]` markers, honest capability note (never a fabricated citation, the done-when); plus abstain pass-through. No src symbol changes beyond the new spec. Prior: RAG-65b `rag_query` MCP tool — new `src/mcp/rag-query.tool.ts` (`registerQueryTool(server, generation)`) registers the corpus Q&A tool over `GenerationService.generate` with a Zod `inputSchema {question}` + `outputSchema` mirroring `QueryResult`; returns `content:[text]` + `structuredContent` (the full `QueryResult`, so agents branch on `grounded`/`abstained` without string-matching — D1). New `src/mcp/render.ts` (`renderQueryResult`) does the text rendering: answer + numbered `Sources:` list `[n] "citedText" — source`; abstain passes through verbatim (D5), non-citation provider gets an honest capability note (never a fabricated marker). `src/mcp/main.ts` now calls `registerQueryTool`. Deviation from the guide's inline `[n]` markers: our `Citation` carries the source span, not an answer offset, so markers are a trailing list. Live-verified via a real stdio MCP `Client`: grounded question → `grounded:true` + 1 chunk + qwen answer; out-of-corpus → `abstained:true`/`grounded:false` + verbatim message. Prior: RAG-65a MCP entrypoint — new `src/mcp/main.ts` (`bootstrap()` self-invoked): a **third entrypoint** over the same in-process services (mirrors `src/cli/main.ts`) that serves an `McpServer` (`@modelcontextprotocol/sdk` v1.30.0, exact-pinned) over `StdioServerTransport`; logs forced to stderr (`CorrelatedLogger('stderr')`, D3) so stdout is a pure JSON-RPC channel; resolves `GenerationService` at boot (fail-fast); `npm run mcp` script. Prior: RAG-66d `rag init` scaffold writer — new `src/cli/init.ts` (`scaffoldFiles()` pure file-set data + `runInit()` fs orchestrator: idempotent skip-existing, `--force`, `--dry-run`) and `formatInitResult` in `src/cli/format.ts`; third `commander` subcommand in `src/cli/main.ts` (no app-context bootstrap — file-writing only). Writes `src/rag/rag.module.ts` (host wiring importing `RagModule` from the package), `.env.rag.example`, `docker-compose.rag.yml` (standalone pgvector on port 5433), `db/rag/001_init.sql` (documents reusing the shipped migration runner via `MIGRATIONS_DIR` env override). Live-smoke-verified: `node dist/cli/main.js init` and `npx rag init` both scaffold; re-run skips; `--force` overwrites; `--dry-run` writes nothing; generated compose YAML validates. Prior: RAG-66c typed `forRoot(options)` override surface — `RagModuleOptions` (`embeddingProvider`/`generationProvider`/`k`/`minScore`/`http`); `EmbeddingModule`/`GenerationModule`/`IngestionModule`/`RetrievalModule` each gained a `register()` static (the override-aware path) alongside their existing decorator-based default. `GenerationModule`/`IngestionModule` decorators had `imports`/`controllers` **removed** (moved into `register()` only) — Nest concatenates a class's static `@Module()` metadata with a `DynamicModule`'s own `imports`/`controllers` rather than replacing them, so a decorator-level `imports:[RetrievalModule]` silently fought `register()`'s override-aware import (see `generation.module.ts` doc comment); `providers` concatenation is safe (token-keyed, last wins) so that pattern was kept for `embeddingProvider`/`generationProvider`. `AppModule` now wires `IngestionModule.register({http:true})` / `GenerationModule.register({http:true})` (its own bare `RetrievalModule` import removed as redundant) — `register()` is the single source of truth for both the standalone app and the embedded surface. `RetrievalService` gained an `@Optional` `RAG_RETRIEVAL_OPTIONS` token (k/minScore override, env-fallback unchanged). Prior: RAG-66b embeddable surface — `src/index.ts` barrel + `src/rag.module.ts` (`RagModule.forRoot()`), `package.json` importable (`private:false`, `main`/`types`/`exports`/`files`), `tsconfig` `declaration:true`.
...
### `src/mcp/main.ts`
- **Purpose:** The MCP server entrypoint (GO-21i / RAG-65) — a third entrypoint over the same in-process services as the HTTP API and CLI. Bootstraps `NestFactory.createApplicationContext(AppModule, { logger: false })` (same pattern as `src/cli/main.ts`), forces logs to **stderr** via `CorrelatedLogger('stderr')` (D3), resolves `GenerationService` + `IngestionService` at boot (fail-fast), reads config via `ConfigService` (`MCP_ENABLE_INGEST` gate + `resolveMcpServerConfig` for transport). Serves the `registerTools` set over the selected transport: **stdio** (`StdioServerTransport`) or **http** (`startHttpServer` — Streamable HTTP + bearer, RAG-65e). SIGINT/SIGTERM → clean shutdown (closes the http listener or stdio server, then `app.close()`). Registered as `npm run mcp` (`node dist/mcp/main.js`).
- **Defines:** `bootstrap(): Promise<void>` (self-invoked) · `SERVER_INFO` (file-private const `{ name:'rag', version:'0.1.0' }`)
- **Depends on:** `AppModule`, `GenerationService`, `IngestionService`, `ConfigService` (`@nestjs/config`), `CorrelatedLogger` (`../observability/correlated-logger`), `Logger`, `NestFactory`, `McpServer`/`StdioServerTransport` (`@modelcontextprotocol/sdk`), `registerTools`+`McpToolDeps` (`./register-tools`), `resolveMcpServerConfig` (`./config`), `startHttpServer` (`./http-transport`)
- **Used by:** — (entrypoint; invoked as `node dist/mcp/main.js` / `npm run mcp`)

### `src/mcp/config.ts`
- **Purpose:** Resolves the MCP transport config from env (RAG-65e). Pure (takes a getter, no `ConfigService`) so it's unit-testable. **Fail-closed:** `MCP_TRANSPORT=http` without `MCP_AUTH_TOKEN` throws, so the server never serves MCP over the network without a bearer (D4); stdio stays unauthenticated by design.
- **Defines:** `resolveMcpServerConfig(get: (key: string) => string | undefined): McpServerConfig` · `McpServerConfig` (interface: `transport`, `httpPort`, `authToken`) · `McpTransport` (type `'stdio' | 'http'`) · `DEFAULT_MCP_HTTP_PORT` (3001)
- **Depends on:** — (pure)
- **Used by:** `src/mcp/main.ts` · `src/mcp/config.spec.ts`

### `src/mcp/http-transport.ts`
- **Purpose:** Streamable HTTP transport for the MCP server (RAG-65e, design guide §6) — a self-contained minimal `http.createServer` listener so the MCP layer stays a third entrypoint independent of the web app. Every request to `/mcp` is bearer-gated (`isAuthorized`, D4) before any pipeline work; other paths 404. **Stateless:** a fresh `McpServer` + `StreamableHTTPServerTransport` (`sessionIdGenerator: undefined`) per request (SDK's stateless pattern); the pipeline services in `deps` are resolved once and reused.
- **Defines:** `isAuthorized(authHeader: string | undefined, token: string): boolean` (constant-time bearer check) · `startHttpServer(opts: HttpServerOptions): Promise<Server>` · `HttpServerOptions` (interface) · `MCP_PATH` (`'/mcp'`)
- **Depends on:** `node:http`, `node:crypto` (`timingSafeEqual`), `McpServer`/`StreamableHTTPServerTransport` (`@modelcontextprotocol/sdk`), `registerTools`+`McpToolDeps` (`./register-tools`), `Logger` (type)
- **Used by:** `src/mcp/main.ts` · `src/mcp/http-transport.spec.ts`

### `src/mcp/register-tools.ts`
- **Purpose:** MCP tool aggregator + the write-capability gate (RAG-65d, D2). `registerTools(server, {generation, ingestion, enableIngest})` registers `rag_query` always and `rag_ingest` only when `enableIngest` is true — so with `MCP_ENABLE_INGEST` off, the ingest tool never appears in `tools/list` and an agent can't trigger a corpus write. Deliberately a pure function (no Nest, no config reads) so the gate is unit-testable without an app context.
- **Defines:** `registerTools(server: McpServer, deps: McpToolDeps): void` · `McpToolDeps` (interface: `generation`, `ingestion`, `enableIngest`)
- **Depends on:** `McpServer`, `GenerationService`, `IngestionService`, `registerQueryTool` (`./rag-query.tool`), `registerIngestTool` (`./rag-ingest.tool`)
- **Used by:** `src/mcp/main.ts` · `src/mcp/register-tools.spec.ts`
- **Tested by (RAG-65d):** `register-tools.spec.ts` — flag off → registers only `rag_query`; flag on → also `rag_ingest`.

### `src/mcp/rag-ingest.tool.ts`
- **Purpose:** Registers the **gated** `rag_ingest` MCP tool (RAG-65d, design guide §3/D2) — chunk/embed/index a filesystem path into the corpus via `IngestionService.ingest` (idempotent). Input `{path}`, output `{docs,chunks,ms}` (Zod `outputSchema`); returns `structuredContent` + a one-line text summary. Only *defines* the tool — the `MCP_ENABLE_INGEST` gate that decides whether it's registered lives in `register-tools.ts`.
- **Defines:** `registerIngestTool(server: McpServer, ingestion: IngestionService): void` · `RAG_INGEST_DESCRIPTION` / `RAG_INGEST_OUTPUT` (file-private)
- **Depends on:** `McpServer`, `IngestionService`, `z` (`zod`)
- **Used by:** `src/mcp/register-tools.ts` · `src/mcp/rag-ingest.tool.spec.ts`

### `src/mcp/rag-query.tool.ts`
- **Purpose:** Registers the `rag_query` MCP tool (RAG-65b, design guide §3) — the corpus Q&A tool. Calls `GenerationService.generate` in-process (no retrieval/abstain/citation logic of its own — abstain-on-empty lives in `generate`, D5). Prescriptive description tells agents to treat `grounded:false` as a real answer, not an error. Declares a Zod `outputSchema` mirroring `QueryResult` so the SDK validates `structuredContent` and advertises the shape (D1). `structuredContent` is spread into a fresh object literal (`{ ...result }`) because the SDK types it as `Record<string, unknown>`, which a named interface doesn't implicitly satisfy.
- **Defines:** `registerQueryTool(server: McpServer, generation: GenerationService): void` · `RAG_QUERY_DESCRIPTION` / `RAG_QUERY_OUTPUT` (file-private)
- **Depends on:** `McpServer` (`@modelcontextprotocol/sdk/server/mcp.js`), `GenerationService`, `renderQueryResult` (`./render`), `z` (`zod`)
- **Used by:** `src/mcp/main.ts` · `src/mcp/rag-query.tool.spec.ts`
- **Tested by (RAG-65c):** `rag-query.tool.spec.ts` — captures the registered tool handler via a stub `McpServer` + stub `GenerationService`; asserts citation serialization for **both provider types** (Anthropic → `citations[]` mapped to sources in `structuredContent` + numbered `Sources:` text; openai-compatible → `citationsSupported:false` + `citations:[]`, no `[n]` markers, capability note) and abstain pass-through. Provider-level citation *mapping* is tested separately (`anthropic-generation.provider.spec.ts` / `openai-compatible-generation.provider.spec.ts`).

### `src/mcp/render.ts`
- **Purpose:** Text rendering of a `QueryResult` for MCP clients that only display text (design guide §4) — the answer, then a numbered `Sources:` list `[n] "citedText" — source`. Abstain passes through verbatim (D5, no Sources section); a non-citation provider (`citationsSupported:false`) gets an explicit capability note instead of markers (never a fabricated citation, RAG-62 / `ai-and-secrets.md`). Markers are a trailing list, not inline — `Citation` carries the source span (`citedText`/`documentIndex`), not an answer offset, so there's no faithful mid-prose position.
- **Defines:** `renderQueryResult(result: QueryResult): string`
- **Depends on:** `QueryResult` (`../generation/generation.service`, type only)
- **Used by:** `src/mcp/rag-query.tool.ts` · `src/mcp/render.spec.ts`
...
| `MCP_ENABLE_INGEST` | `src/mcp/main.ts` (gates registering the `rag_ingest` MCP tool, RAG-65d) | false |
| `MCP_TRANSPORT` | `src/mcp/config.ts` (`resolveMcpServerConfig` — `stdio` \| `http`, RAG-65e) | stdio |
| `MCP_HTTP_PORT` | `src/mcp/config.ts` (HTTP transport port) | 3001 |
| `MCP_AUTH_TOKEN` | `src/mcp/config.ts` (bearer for HTTP; **required** when `MCP_TRANSPORT=http`) · `src/mcp/http-transport.ts` (`isAuthorized`) | — |
