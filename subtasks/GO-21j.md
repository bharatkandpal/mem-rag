...
**Start here:** RAG-66a–d are **done** — approach guide, importable surface, typed
`forRoot(options)`, and the `rag init` scaffold writer (file set live-smoke-verified). **RAG-46
(migration runner) has landed on `main`**, so RAG-66e is unblocked. The next actionable moves are
**RAG-66e** (formalize/verify the migration/DB path — the runner reuse is already documented,
this is about a real end-to-end apply) and **RAG-66f** (the `npm run rag` script alias + README
section), either of which can proceed in parallel; RAG-66g (end-to-end host smoke) needs both.
