...
| GO-21e-h | **Full-polish + a11y pass** — motion budget (reduced-motion aware), focus rings, keyboard nav, AA contrast, responsive/mobile check, observability correlation-id surfacing (RAG-63) | Keyboard-only flow works, `prefers-reduced-motion` honored, AA verified in both themes, correlation id shown on error | GO-21e-e, GO-21e-f — *(done 2026-07-30: `ErrorBoundary` around the conversation; targeted `aria-live` on assistant turns; **computed WCAG AA** in both themes — fixed light `--text-muted` 2.56→4.83 & `--success` 3.00→4.56, dark `--text-muted`→5.24; reduced-motion/focus-rings/correlation-id already banked at the token layer + `ErrorState`)* |
...
(2026-07-29), and **GO-21e-h** (full-polish + a11y pass) is **done** (2026-07-30). ✅
**GO-21e is complete** — all sub-tasks a…i landed; RAG-31/32/33 closed. Remaining nice-to-haves
(deferred, not blocking): mobile bottom-sheet citation popovers (guide §7) and a human visual
sweep of the polished build in both themes.
