...
- **Abstain on empty retrieval.** If no chunk clears the score floor, return "not in the corpus" — never free-generate an answer. This lives in `GenerationService.generate`, above the provider, so it holds regardless of which provider is configured. Grounding is the whole product. This is the **default** and it is non-negotiable.
- **The one sanctioned ungrounded path is explicit and labelled.** `GenerationService.generateGeneral` / `POST /query/general` returns a general-knowledge answer that is *not* from the corpus (`grounded: false`, `citations: []`). It exists only as a **user-initiated opt-in after an abstain** — never the default query path, never automatic — and the caller/UI must present it as non-corpus (the chat UI colour-codes it amber). The default `/query` abstain guarantee above is untouched: making the exception explicit and visible is the point; a silent free-generation fallback would violate this rule.
